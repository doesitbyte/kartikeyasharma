...
export const WavyBackground = ({
  ...
  const waveOpacityRef = useRef(waveOpacity || 0.5);
  const waveWidthRef = useRef(waveWidth || 50);

  // Update refs when props change
  useEffect(() => {
    if (colors) waveColorsRef.current = colors;
    if (backgroundFill) backgroundFillRef.current = backgroundFill;
    if (waveOpacity !== undefined) waveOpacityRef.current = waveOpacity;
    if (waveWidth !== undefined) waveWidthRef.current = waveWidth;
  }, [colors, backgroundFill, waveOpacity, waveWidth]);
